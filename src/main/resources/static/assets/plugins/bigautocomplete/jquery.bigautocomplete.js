    ...
    var bigAutocomplete = new function () {
        ...
        this.init = function () {
            ...
            $("#bigAutocompleteContent").delegate("tr", "click", function () {
                ...
                var fields_ = bigAutocomplete.currentInputText.data("config").fields;
                if ($("#bigAutocompleteContent").css("display") != "none" && callback_ && $.isFunction(callback_)) {
                    callback_($(this).data("jsonData"), fields_);
        ...
        this.autocomplete = function (param) {
            ...
            this.config = {
                ...
                url: null,
                /*data：格式{data:[{field_:null,result:{}},{field_:null,result:{}}]}
            ...
            $this.keyup(function (event) {
                ...
                if (!isFunctionalKey && (!ctrl || (ctrl && k == 67) || (ctrl && k == 88))) {
                    ...
                    if (data != null && $.isArray(data)) {
                        ...
                        for (var i = 0; i < data.length; i++) {
                            if (data[i][field_].indexOf(keyword_.toUpperCase()) > -1) {
                                data_.push(data[i]);
                            }
                        }

                        makeContAndShow(data_, field_);
                    } else if (url != null && url != "") {//ajax请求数据
                        $.post(url, {keyword: keyword_}, function (result) {
                            makeContAndShow(result.data, field_)
                        }, "json")
                    }
                ...
                if (k == 13) {
                    ...
                    var fields_ = $this.data("config").fields;
                    if ($bigAutocompleteContent.css("display") != "none") {
                        if (callback_ && $.isFunction(callback_)) {
                            callback_($bigAutocompleteContent.find(".ct").data("jsonData"), fields_);
                        }
            ...
            //组装下拉框html内容并显示
            function makeContAndShow(data_, field_) {
                ...
                for (var i = 0; i < data_.length; i++) {
                    cont += "<tr><td><div>" + data_[i][field_] + "</div></td></tr>"
                }
                cont += "</tbody></table>";
